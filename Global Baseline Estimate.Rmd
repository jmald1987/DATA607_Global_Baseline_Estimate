...
recommendations <- baseline_estimates %>%
  arrange(name, desc(estimated_rating))
...
The Global Baseline Estimate uses the overall rating average along with differences in individual user behavior and movie ratings to estimate ratings that were missing from the original survey. This provides a non-personalized baseline recommendation method while still accounting for whether a particular user tends to rate titles higher or lower and whether a particular title tends to receive ratings above or below the overall average.

The highest estimated ratings can be used to identify potential recommendations for each user. The analysis could be extended by collecting additional ratings, comparing estimated ratings with actual ratings collected later, or comparing the Global Baseline Estimate with more personalized recommendation methods.
